...
function make_lilypad_water_lane () {
    for (let col = 0; col <= tiles.tilemapColumns() - 1; col++) {
        ...
        if (Math.percentChance(50)) {
            ...
            tiles.setTileAt(tiles.getTileLocation(col, 0), [
            assets.tile`lilypad_up`,
            assets.tile`lilypad_down`,
            assets.tile`lilypad_left`,
            assets.tile`lilypad_right`,
            assets.tile`lily`
            ]._pickRandom())
        }
    }
...
function make_images () {
    image_title_screen = blockObject.getImageProperty(make_multi_language_image(assets.image`title_screen2`, assets.image`title_screen`, assets.image`title_screen_spanish`), image_lang)
    ...
    if (option_selected == 0) {
        ...
        while (true) {
            ...
            if (blockMenu.selectedMenuIndex() == 1) {
                ...
                blockMenu.showMenu([
                get_text("     Select language", "   选择一种语言", "   Selecionar lenguage"),
                get_text("English", "英文", "Ingles"),
                get_text("Chinese", "中文", "Chino"),
                get_text("Spanish", "西班牙语", "Español")
...
game.onUpdateInterval(1000, function () {
    if (Math.percentChance(50)) {
        if (tiles.getTilesByType(assets.tile`road_right`).length > 0) {
            sprite_car = sprites.create([
            assets.image`red_right_facing_car`,
            assets.image`blue_right_facing_car`,
            assets.image`pink_right_facing_car`,
            assets.image`autoR`
            ]._pickRandom(), SpriteKind.Enemy)
        ...
        if (tiles.getTilesByType(assets.tile`road_left`).length > 0) {
            sprite_car = sprites.create([
            assets.image`red_left_facing_car`,
            assets.image`blue_left_facing_car`,
            assets.image`pink_left_facing_car`,
            assets.image`autoL`
            ]._pickRandom(), SpriteKind.Enemy)
